...
【TODO】待实现内容(对应课程 demo_1~demo_11):
    - /status              连通性 + 集群信息(demo_1 连接)
    - /analyze             IK 分词演示:ik_max_word vs ik_smart
    - 索引管理:创建带 IK mapping 的索引 / 删除索引(demo_2 建索引)
    - 文档写入:单条 / bulk 批量(demo_3 / demo_4)
    - 文档检索:match 中文查询 + 高亮(demo_6)
    - 复合查询:Bool / 分页(demo_7 / demo_9)
"""
