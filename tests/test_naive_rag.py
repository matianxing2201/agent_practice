"""Naive RAG 检索 + 生成流程测试(流式 SSE)。

数据通过 knowledge_base 接口(/rag/knowledge/*)写入,本方案只消费。
"""
...
def test_naive_rag_query_empty(client):
    resp = client.post("/rag/naive/query", json={"query": ""})
    assert resp.status_code == 400


def test_naive_rag_query_stream(client, fake_embedding):
    resp = client.post("/rag/naive/query", json={"query": "感冒症状"})
...
def test_naive_rag_full_flow(client, fake_embedding):
    content = (
        "1 感冒・风寒束表证\n主诉:恶寒头痛 2 天。\n\n"
        "2 咳嗽・痰湿蕴肺证\n主诉:咳嗽痰多 2 周。\n"
    )
